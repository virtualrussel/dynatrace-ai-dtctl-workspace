...
### Fixed
- **`setup.sh`: doc-substitution idempotency check never actually ran.** The check gated on `CLAUDE.md` containing `YOUR_TENANT_ID`, but `CLAUDE.md` never contained that placeholder (it only names the MCP server, never a URL) — so the check always evaluated "already configured," and `README.md`/`ARCHITECTURE.md`/`docs/ELI5.md`/`docs/CHEATSHEET.md` never actually got the user's tenant substituted in, on any run, including the very first. Fixed by gating on `README.md`, which reliably contains the placeholder pre-substitution.
- **`setup.sh`: idempotency check false-triggered on its own example code.** After the fix above, the check (`grep -q "YOUR_TENANT_ID"`) matched a bare `YOUR_TENANT_ID` inside README's own manual-regeneration example command, causing the opposite failure — it always reported "not yet configured" even after a successful run. Fixed by requiring the full placeholder pattern (`YOUR_TENANT_ID.apps.dynatrace.com` / `YOUR_TENANT_ID.sprint.apps.dynatracelabs.com`), which only matches genuine unsubstituted placeholders, not the bare token used in example code.
- **`setup.sh`: reconfiguring required retyping the full tenant URL, and silently desynced docs from config if it changed.** Re-running `setup.sh` on an already-configured clone now detects and displays the existing tenant; pressing Enter keeps it (for Platform Token rotation without retyping the domain). If a different tenant is entered, the script now explicitly warns that the generated config and the documentation would otherwise describe different tenants, and offers to retarget the documentation to match before proceeding.
...
- `README.md`: restructured the setup flow to front-load Platform Token creation and its required scopes; rewrote the "MCP Configuration Files" section to describe the template/generated split; updated the file-tree diagram to show `.template` files instead of the old live config files; corrected the described `setup.sh` step order to match actual execution (scopes and token are collected before documentation substitution runs); documented the reconfigure/rotation flow.
- `docs/ELI5.md`, `README.md`: documented that Claude Code CLI requires a one-time, per-clone approval before it will connect to a new project's MCP server (`claude mcp list` shows "Pending approval" until approved in an interactive session) — Claude Code's own trust mechanism, not specific to this workspace, but previously undocumented here.
